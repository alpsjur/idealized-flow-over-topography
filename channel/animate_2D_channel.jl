...
vₙ = @lift interior(v_timeseries[$n], 1, 1:ystep:length(yc), 1:zstep:length(zc))
wₙ = @lift interior(w_timeseries[$n], 1, 1:ystep:length(yc), 1:zstep:length(zc))
...
CairoMakie.record(fig, "channel/animations/2D_BLOM_channel.mp4", frames, framerate=8) do i
