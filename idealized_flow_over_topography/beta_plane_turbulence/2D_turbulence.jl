...
ω_timeseries = FieldTimeSeries("data/"*filename*".jld2", "ω")
s_timeseries = FieldTimeSeries("data/"*filename*".jld2", "s")
u_timeseries = FieldTimeSeries("data/"*filename*".jld2", "u")
v_timeseries = FieldTimeSeries("data/"*filename*".jld2", "v")
U_timeseries = FieldTimeSeries("data/"*filename*".jld2", "U")
PV_timeseries = FieldTimeSeries("data/"*filename*".jld2", "PV")
...
#ax_u = Axis(fig[2,2]; title="zonal velocity", axis_kwargs...)
#ax_v = Axis(fig[2,3]; title="meridional velocity", axis_kwargs...)
...
#heatmap!(ax_u, xu, yu, u; colormap=:balance, colorrange=(-0.2,0.2))
#heatmap!(ax_v, xv, yv, v; colormap=:balance, colorrange=(-0.2,0.2))
...
record(fig, "animations/"*filename*".mp4", frames, framerate=24) do i
    n[] = i
end